...
def test_create_submission_zip_keeps_scene_directories_and_pngs(tmp_path: Path):
    ...
    _write_scene_outputs(submission, "scene_001", 20)
    _write_scene_outputs(submission, "scene_002", 20)
    ...
    assert "scene_002/target_019.png" in names


def test_create_submission_zip_strict_contest_rejects_scene_with_too_few_targets(tmp_path: Path):
    submission = tmp_path / "submission"
