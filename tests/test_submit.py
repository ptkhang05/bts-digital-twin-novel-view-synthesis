...
def test_submit_module_cli_rejects_timestamped_zip_name(tmp_path: Path):
    data_root = tmp_path / "data"
    scene = data_root / "scene_a"
    (scene / "train" / "images").mkdir(parents=True)
    (scene / "train" / "sparse" / "0").mkdir(parents=True)
    ...
    submission.mkdir(parents=True)
    Image.new("RGB", (8, 6)).save(submission / "target.JPG", format="JPEG")
    output = tmp_path / "submission-round2.zip"
    env = os.environ.copy()
    env["PYTHONPATH"] = str(Path(__file__).parents[1] / "src")
    ...
    assert "single repository-root submission.zip" in completed.stderr
